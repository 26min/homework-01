...
## Декораторы
В проекте реализован модуль `decorators`, который включает:
- **Декоратор `log`**: 
  - Автоматически регистрирует начало и конец выполнения функций.
  - Логирует результат успешного выполнения или тип ошибки и входные аргументы, если произошел сбой.
  - Поддерживает запись логов как в **консоль**, так и в **файл**.
...
def my_function(x, y):
    return x + y
```
Это позволит обеспечить более 
глубокий контроль и анализ поведения программы в 
